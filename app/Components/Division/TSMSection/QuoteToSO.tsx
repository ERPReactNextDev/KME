...
const QuoteToSO: React.FC<QuoteToSOProps> = ({
  filteredProgress,
  tsaList,
  loading,
}) => {
  ...
  const perTsaStats = useMemo(() => {
    return tsaList.map((tsa) => {
      ...
      const so = filteredProgress.filter(
        (r) => r.referenceid === tsa.ReferenceID && r.activitystatus === "SO-Done"
      ).length;

      const conversionRate = quotes > 0 ? ((so / quotes) * 100).toFixed(1) : "0";

      return { tsa, quotes, so, conversionRate };
    });
  ...
  const totalQuotes = filteredProgress.filter(
    (p) => p.activitystatus === "Quote-Done"
  ).length;
  const totalSO = filteredProgress.filter(
    (p) => p.activitystatus === "SO-Done"
  ).length;
  const totalConversion =
    totalQuotes > 0 ? ((totalSO / totalQuotes) * 100).toFixed(1) : "0";
  ...
  return (
    <div className="bg-white shadow-md rounded-lg p-6">
      ...
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-bold">Quote to SO</h2>
      </div>
      ...
      {/* 🧾 Table */}
      <Table>
        <TableCaption>Quote-to-SO performance by TSA.</TableCaption>
        <TableHeader>
          <TableRow>
        ...
        <TableBody>
          {loading ? (
            ...
            rankedTSA.map(({ tsa, quotes, so, conversionRate }, index) => (
              <TableRow key={tsa._id} className="hover:bg-gray-50 cursor-pointer">
                ...
                <TableCell className="text-right">{so}</TableCell>
